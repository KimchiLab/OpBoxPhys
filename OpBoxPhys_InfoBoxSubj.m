...
dir_orig = pwd; % Assume Info.csv files are in the same directory
filename = 'InfoBoxes.csv';
file_data = csvimport(filename); % http://uk.mathworks.com/matlabcentral/fileexchange/23573-csvimport
% Can substitute with readtable, but only introduced in Matlab R2013b
...
box_info = struct('room', file_data(2:end,col_room), 'name', file_data(2:end,col_name), ...
    ...
    'ch_offset', CellStrToCellNums(file_data(2:end,col_ch_offset)), 'ch_trigger', CellStrToCellNums(file_data(2:end,col_ch_trigger)), ...
